...
a = pd.read_csv(path + r"SimulationResults\\RData_SmallWorldNewman.csv")
...
for p in ShortCutProbs:
    ...
    plt.savefig(path + r"SimulationResults\\FOI_Pics\\SmallWorldNewman\\SmallWorldNewmanIncidence_SCP_" + str(np.round(p,4)) + ".png")
    plt.close()
    del fig
    del ax
